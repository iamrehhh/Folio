...
const VISIBILITY_OPTIONS: {
...
}[] = [
  {
    value: 'private',
    label: 'Private',
    description: 'Only you can see these books',
    icon: Lock,
    color: '#6B7280',
    bgColor: '#6B728015',
  },
  ...
  },
  {
    value: 'assigned',
    label: 'Assigned',
    description: 'Only selected users can see them',
    icon: UserCheck,
    color: '#3B82F6',
    bgColor: '#3B82F615',
  },
...
export default function BulkBookAccessModal({ bookIds, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    // We can use the first bookId to get the full users list. We ignore 'assigned' status.
    fetch(`/api/admin/books/${bookIds[0]}/access`)
      .then(r => r.json())
      .then(d => {
        setUsers(d.users ?? []);
      })
    ...
    try {
      if (visibility === 'assigned') {
        const res = await fetch('/api/admin/books', {
          ...
          body: JSON.stringify({ bookIds, userIds: Array.from(selectedIds) }),
        });
        if (!res.ok) throw new Error('Failed to update bulk assignments');
      } else {
        // Clear assignments (which reverts to private)
        const putRes = await fetch('/api/admin/books', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ bookIds, userIds: [] }),
        });
        if (!putRes.ok) throw new Error('Failed to clear previous assignments');

        // Update to desired visibility
        const patchRes = await fetch('/api/admin/books', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ bookIds, visibility }),
        });
        if (!patchRes.ok) throw new Error('Failed to update bulk visibility');
      }

      toast.success(`Access updated for ${bookIds.length} books`);
      onSaved();
    } catch (err: any) {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
          <div className="min-w-0">
            ...
            <p className="text-sm truncate mt-0.5" style={{ color: 'var(--text-secondary)' }}>
              Applying to {bookIds.length} selected book{bookIds.length !== 1 ? 's' : ''}
            </p>
          </div>
